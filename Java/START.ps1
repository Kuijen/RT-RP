# RT-RP V2.6.0 - START.ps1 (Java Edition)
...
$gni        = Join-Path $scriptDir "gnirehtet.exe"
$gniJar     = Join-Path $scriptDir "gnirehtet.jar"
...
while ($restart) {
    ...
    if (G 'Enable_RT') {
        ...
        if (-not (Test-Path $gni)) {
            LErr "gnirehtet.exe not found in $scriptDir"
            $gniOk = $false
        } else {
    ...
    if (G 'Enable_RT') {
        if (-not (Test-Path $gniJar)) {
            LErr "gnirehtet.jar not found in $scriptDir"
        } else {
            & java -jar $gniJar relay 2>&1 | Where-Object { $_ -match '(WARN|ERROR)' } | Add-Content $logFile
        }
